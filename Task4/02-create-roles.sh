...
rules:
  ...
  - apiGroups: [""]
    resources: ["namespaces"]
    resourceNames: ["sales", "tenant-services", "finance", "data-platform"]
    verbs: ["patch"]
---
  ...
  matchConstraints:
    resourceRules:
      - apiGroups: [""]
        ...
        resources: ["namespaces"]
  matchConditions:
    - name: propdevelopment-domain-namespaces-only
      expression: object.metadata.name in ['sales', 'tenant-services', 'finance', 'data-platform']
